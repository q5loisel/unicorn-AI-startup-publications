...
import re  # Used for "regular expressions" (text cleaning) to help deduplicate titles
...
# Only search the INSPEC database.
TARGET_DATABASES = {
    "INSPEC": "INSPEC",
}
...
def main():
    ...
    for affil in AFFILIATIONS:
        ...
        for db_id in TARGET_DATABASES.keys():
            ...
            for record in records:
                ...
                # IF WE ALREADY HAVE THIS PAPER: merge the startup name and skip re-extraction.
                ...
                # Check whether any author has addr_no linking (WoS-style).
                # INSPEC records often omit addr_no entirely, so we fall back to
                # assigning all document addresses to every author in that case.
                any_addr_no = any(
                    bool(n.get("addr_no", "")) for n in names_list if isinstance(n, dict)
                )
                all_addresses_fallback = list(address_mapping.values())
                ...
                for name in names_list:
                    if isinstance(name, dict) and name.get("role") == "author":
                        ...
                        if display_name:
                            ...
                            # INSPEC fallback: if no addr_no linking exists at all but we
                            # do have document-level addresses, assign them to every author.
                            if not author_addrs and not any_addr_no and all_addresses_fallback:
                                author_addrs = all_addresses_fallback
    ...
    # ---------------------------------------------------------
    csv_filename = os.path.join(DATA_DIR, "inspec_papers.csv")
